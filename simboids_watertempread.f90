program simulator
	...
	double precision	:: Ka=0., deltat=20., size=2000., time=1., bl=0.3	!拡散係数/タイムステップ/グリッドサイズ/計算時間(h)/マアジの体長
...
10    continue
    ...
    x1fish(1,1,n) = size * (ny-1) * random1
    y1fish(1,1,n) = size * (nx-1) * random2
    fi(n) = int(x1fish(1,1,n)/size) + 1
    fj(n) = int(y1fish(1,1,n)/size) + 1
    if (ground(fj(n), fi(n)) /= 1) goto 10  ! 陸地ならやり直し
enddo
...
!	write(*,*) temp(3)
    ...
    !――――――――――――――――――――
!   水温選好強度を計算
!   新方式：bTMP1-00012.dat（海域のみのデータ）を読み込む
	open(60, file='input/bTMP1-00012.dat', status="old", action="read")
...
close(60)
! 水温読み込み直後のチェック
!write(*,*) 'para(1,10,10) = ', para(1,10,10)

! === 水温ヒートマップ用CSV出力 ===
open(unit=90, file='output/temptest.csv', status='replace', action='write')
do i = 1, nx
  do j = 1, ny
    write(90,'(I4,",",I4,",",F8.3)') i, j, para(1,i,j)
  enddo
enddo
close(90)
...
75	format('startresult/min', I5.5, '.csv')
76  format('startresult/para_num/fish_number.csv')
77  format(3(I6),1(f10.8))
78  format('startresult/para_num/parameter.csv')
...
!	
	open(unit=76, file='startresult/para_num/fish_number.csv', status='old', position='append', action='write')
	...
	open(unit=78, file='startresult/para_num/parameter.csv', status='replace', action='write', encoding='UTF-8')
