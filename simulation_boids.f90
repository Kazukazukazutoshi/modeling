...
!	地形読み込み
	open(10,file='input/umichikei.txt')
	do i = 1, mx
...
!   水温選好強度を計算
!	水温データの読み込み
    open(60,file='input/temp12-30.csv')
    do i = 1, 2500	!読み込みデータの行数総数
...
75	format('result/min', I4.4, '.csv')
76  format('result/para_num/fish_number.csv')
77  format(3(I6),1(f10.8))
78  format('result/para_num/parameter.csv')
...
!	
	open(unit=76, file='result/para_num/fish_number.csv', status='old', position='append', action='write')
	...
	open(unit=78, file='result/para_num/parameter.csv', status='replace', action='write', encoding='UTF-8')
